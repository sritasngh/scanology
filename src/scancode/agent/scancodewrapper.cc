...
string scanFileWithScancode(const State& state, const fo::File& file)
{
  FILE* in;
  char buffer[512];
  string command = "scancode " + file.getFileName();
  string result;
...
{
  string licensePart = extractLicensePartFromScancodeResult(scancodeResult);
  return splitLicensePart(licensePart);
}

// Scancode result format: filename;license1,license2,...,licenseN;details...
string extractLicensePartFromScancodeResult(string scancodeResult)
{
  string delimiters = ";\r\n";

  size_t first = scancodeResult.find_first_of(delimiters);
  size_t last = scancodeResult.find_first_of(delimiters, first + 1);

  return scancodeResult.substr(first + 1, last - 1 - first);
}
...
vector<LicenseMatch> createMatches(vector<string> scancodeLicenseNames)
{
  ...
  for (vector<string>::const_iterator it = scancodeLicenseNames.begin(); it != scancodeLicenseNames.end(); ++it)
  {
    const string& scancodeLicenseName = *it;
    if(isLicenseCollection(scancodeLicenseName,matches))
    {
      continue;
    }    
    string fossologyLicenseName = mapLicenseFromScancodeToFossology(scancodeLicenseName);
    unsigned percentage = (scancodeLicenseName.compare("NONE") == 0 || scancodeLicenseName.compare("UNKNOWN") == 0) ? 0 : 100;
...
/*
 * unmatched in lib/Scancode/rules.dict:
 * 
