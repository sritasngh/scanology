...
class ScancodeScheduledTest extends \PHPUnit\Framework\TestCase
{
  /** @var TestPgDb */
  private $testDb;
  /** @var DbManager */
  ...
  {
    $this->testDb = new TestPgDb("scancodeSched".time());
    $this->dbManager = $this->testDb->getDbManager();

    $this->licenseDao = new LicenseDao($this->dbManager);
    $logger = new Logger("ScancodeSchedulerTest");
  ...
  private function runScancode($uploadId)
  {
    ...
    $this->assertTrue($pipeFd !== false, 'running scancode failed');
  ...
  public function testRun()
  {
    ...
    list($output,$retCode) = $this->runScancode($uploadId=1);

    $this->rmRepo();

    $this->assertEquals($retCode, 0, 'scancode failed: '.$output);
    ...
    foreach($matches as $licenseMatch) {
      ...
      $this->assertEquals($agentRef->getAgentName(), "scancode");
    }
  }
